...
function pickLayout({ themePack, role, used, body, mediaIntent = null, seed = null }) {
  ...
  // Long decks may legitimately outnumber the compatible layouts in a theme.
  // Prefer a fresh layout, then reuse a compatible one instead of failing the
  // whole generation after earlier pages have already been streamed.
  const unused = compatible.filter(layout => !used.has(layout));
